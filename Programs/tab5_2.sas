...
* Program Type: Table
*
* Purpose: To produce Table 14.1.7 Subject Demographics -Sex and Race (ITT Population)
* Usage Notes: 
*
* SAS  Version: 9.4
...
data pct1;
   set pct;
   length grp $100.;
   if count>0 then grp= strip(put (count,4.))||" ("|| strip(put (count/denom*100,z4.1))||")";
   else grp='';
run;
...
title3 j=c 'Table 14.1.7 Subject Demographics -Sex and Race (ITT Population)';

footnote1 j=l 'C:\C1\ST\Jul2022\CL\program\tab5_2.SAS';


options orientation=landscape;
ods escapechar='^';
ods rtf file='C:\C1\ST\Jul2022\CL\OUTPUTS\t_14_1_7.rtf' style=styles.test;
...
/*   maintain order for column two statistic*/
   ...
   style (column)={just=r cellwidth=10% asis=on}
   style (header)={just=r cellwidth=10% asis=on}
   ;
